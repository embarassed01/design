...
{
    _alloc_t * allocator = &crack->pt_allocator;
    crack->algo.len_hash = sizeof(hmac_md5_hash_t);
    ...
    hmac_md5_hash_t * hash = (hmac_md5_hash_t*)crack->algo.algo_hash;
    // 1. hash = hmac_md5(key=pwd, in=salt):
    hmac_md5((u8*)password, len_pwd, crack->data.salt, crack->data.salt_len, hash);
    // 2. cmp out vs digest:
