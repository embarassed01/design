...
STATE_FLAG algo150_parse_hash    (crack_t * crack, const char * hash,     int len_hash)
{
    ...
    // 1. len is match?
    if (len_hash < 20 * 2 || len_hash > 20 * 2 + 1 + 256)
    {
...
{
    _alloc_t * allocator = &crack->pt_allocator;
    crack->algo.len_hash = sizeof(hmac_sha1_hash_t);
    ...
    hmac_sha1_hash_t * hash = (hmac_sha1_hash_t*)crack->algo.algo_hash;
    // 1. hash = hmac_sha1(key=pwd, in=salt):
    hmac_sha1((u8*)password, len_pwd, crack->data.salt, crack->data.salt_len, hash);
    // 2. cmp out vs digest:
