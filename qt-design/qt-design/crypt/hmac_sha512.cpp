...
{
    hmac_sha512_ctx_t * ctx = (hmac_sha512_ctx_t*)malloc( sizeof(hmac_sha512_ctx_t) );
    memset((void*)ctx, 0x00, sizeof(hmac_sha512_ctx_t));
    return ctx;
}

hmac_sha512_hash_t * hmac_sha512_hash_create()
{
    hmac_sha512_hash_t * hash = (hmac_sha512_hash_t*)malloc( sizeof(hmac_sha512_hash_t) );
    memset((void*)hash, 0x00, sizeof(hmac_sha512_hash_t));
    return hash;
}
...
void hmac_sha512_start(hmac_sha512_ctx_t * ctx, const u8 * key, size_t key_len)
{
    ...
    {
        sha512_start(&ctx->ctx);
        sha512_update(&ctx->ctx, (u8*)key, key_len);
    ...
    sha512_start(&ctx->ctx);
    sha512_update(&ctx->ctx, ipad, sizeof(ctx->ctx.buffer));
}

void hmac_sha512_update(hmac_sha512_ctx_t * ctx, const u8 * input, size_t inlen)
{
    sha512_update(&ctx->ctx, (u8*)input, inlen);
}
...
void hmac_sha512_final(hmac_sha512_ctx_t * ctx, hmac_sha512_hash_t * digest)
{
    ...
    sha512_final(&ctx->ctx, &hash);
    sha512_start(&ctx->ctx);
    sha512_update(&ctx->ctx, opad, sizeof(ctx->ctx.buffer));
    sha512_update(&ctx->ctx, hash.bytes, sizeof(hash));
    sha512_final(&ctx->ctx, &hash);
...
{
    hmac_sha512_ctx_t ctx;
    hmac_sha512_start(&ctx, key, key_len);
    hmac_sha512_update(&ctx, input, inlen);
    hmac_sha512_final(&ctx, digest);
}
